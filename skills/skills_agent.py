...
from skills.skills_functions import FUNCTION_IMPLS, get_memory_tool_schemas, get_search_tool_schemas
...
class SkillAgent:
    ...
    def __init__(self, agent_config: dict = None, save_process: bool = False, out_dir: str = None, is_template: bool = False) -> None:
        ...
        self.enable_thinking = agent_config.get('enable_thinking', False)
    ...
    def initialize(self, out_dir: str) -> int:
        ...
        if os.path.exists(f"{out_dir}/agent_state.json"):
            with open(f"{out_dir}/agent_state.json", "r") as f:
                ...
                self.memory.skills = state.get('skills', [])
                self.conversation_history = state.get('conversation_history', [])

                # Load embedding IDs
                self.memory.skills_embedding_titles = state.get('skills_embedding_titles', [])
                ...
                if os.path.exists(embeddings_file):
                    ...
                    try:
                        ...
                        self.memory.skills_embedding_matrix = embeddings_data.get('skills_matrix', np.empty((0, 1536)))
...
Please only return the query, no other text."""
    ...
    def chat(self, user_msg: str='', status: str='chat', verbose: bool=False, return_step_info: bool=False):
        ...
        while True:
            ...
            messages = self.memory.render_system_prompt(status=status, query=query, max_num_of_recent_chunks=SkillAgent.MAX_MEMORY_ITEMS) # When memorizing, system_prompt includes self.MEMORY_CONSOLIDATE_STEP memories; When answering, system_prompt includes all memories;
    ...
    def _chat_with_external_model(self, user_msg: str, verbose: bool = False, return_step_info: bool = False, step_info: Dict[str, Any] = None):
        ...
        try:
            ...
            total_memory_count = len(self.memory.skills)
            max_chunks = max(total_memory_count, 1000)  # Use large number to get all memories

            # Manually construct system prompt for SkillMemory as it has a generic 'chat' prompt
            all_skills = self.memory.skills
            skills_block = self.memory._block(title="SKILLS", lines=all_skills)

            system_prompt = (
                "You are a reasoning assistant. Use the skills below to provide accurate, relevant, and comprehensive responses to user queries.\n\n"
                f"AVAILABLE SKILLS:\n{skills_block}\n\n"
                "INSTRUCTIONS:\n"
                "- Use the skills above to inform your responses.\n"
                "- If information is available in a skill, reference it appropriately.\n"
                "- If skills are insufficient to answer a question, acknowledge this clearly."
            )
    ...
    def _recalculate_embeddings(self):
        """Recalculate embeddings for skills memory"""
        import numpy as np

        skill_list = self.memory.skills
        if not skill_list:
            return

        print(f"Recalculating embeddings for skills memory ({len(skill_list)} items)...")

        # Reset embedding data
        self.memory.skills_embedding_matrix = np.empty((0, 1536))
        self.memory.skills_embedding_titles = []

        # Recalculate embeddings for each skill
        for skill in skill_list:
            title = skill.get('title', '')
            content = skill.get('content', '')
            embedding_text = f"Title: {title}\nContent: {content}"
            embedding = self.memory._get_embedding(embedding_text)

            # Add to embedding matrix
            self.memory.skills_embedding_matrix = np.vstack([self.memory.skills_embedding_matrix, embedding.reshape(1, -1)])
            self.memory.skills_embedding_titles.append(title)

        print(f"Finished recalculating skills embeddings")
    ...
    def _save_embeddings_to_npz(self, out_dir: str):
        ...
        embeddings_file = f"{out_dir}/embeddings.npz"
        np.savez_compressed(embeddings_file, skills_matrix=self.memory.skills_embedding_matrix)

        print(f"Saved embeddings to {embeddings_file}")

        # Also update the embedding titles in the JSON state file
        import json
        ...
        if os.path.exists(state_file):
            ...
            state['skills_embedding_titles'] = self.memory.skills_embedding_titles
            ...
            if isinstance(function_call["arguments"], str):
                ...
                if name in ["new_memory_insert", "memory_update"]:
                    ...
                    if content_key in arguments and not isinstance(arguments[content_key], str):
                        ...
                        raw_val = arguments[content_key]
                        arguments[content_key] = str(raw_val)
    ...
    def process_text_with_qwen_pipeline(text: str, tokenizer, functions=None, status='memorie', enable_thinking=True, max_length=None, memory=None, device=None, max_num_of_recent_chunks=None, return_text=False):
        ...
        if memory is not None:
            ...
            query = text
            max_num_of_recent_chunks = max_num_of_recent_chunks if max_num_of_recent_chunks is not None else SkillAgent.MAX_MEMORY_ITEMS
