...
"""
            ...
            system_prompt = (f''' ### Memory System Optimization Protocol

**Objective**: Maintain and refine an evolving repository of problem-solving memories. Extract high-value, reusable patterns, insights and strategies to enhance long-term performance. This protocol guides the selective retention, organization, and consolidation of information based on its value and relevance to future tasks.          

---                             
## Core Responsibilities
                             
1. **Selective Knowledge Retention:** Preserve only high-value strategies, insights, and reusable patterns. Discard redundant or problem-specific details.       
2. **Synthesis:** Analyze new inputs for lessons, tricks, and errors. Update or replace existing memories with improved versions when superior strategies are identified.
3. **Structure:**
    {"* planning_memory: Compressed meta-reasoning protocols and strategic plannings stored so far (stored as a compact paragraph)" if self.including_core else ""}
    * knowledge_memory: General knowledge or principles; factual or conceptual information;
    * skills_memory: Concrete skills that may include useful insights, reusable strategies, or specific experiences

## Execution Steps
For every new experience or content encountered:
1.  **Evaluate:** Read the problem and attempted solution, evaluate the effectiveness of existing strategies. Is the new strategy optimal or more efficient than current records?
2.  **Extract:** Identify key heuristics and edge cases worth retaining.
3.  **Refine:** Create concise, actionable entries. Replace old versions if a better method is found.

---
## CURRENT MEMORY STATE:
{core_memory_section}
<knowledge_memory> (Only show the most recent {min(len(semantic_items), len(self.semantic))} out of {len(self.semantic)} memories)
{self._block(lines=semantic_items)}
</knowledge_memory>

<skills_memory> (Only show the most recent {min(len(episodic_items), len(self.episodic))} out of {len(self.episodic)} memories)
...
Meanwhile, you will be queried only once, so make sure to call all the memory functions in one turn.''')
...
{search_instructions}"""
    ...
    def new_memory_insert(self, memory_type: str, content: str):
        ...
        #     # Skip insertion for empty content
        #     raise ValueError("Cannot insert empty content into memory.")
    ...
    def memory_update(self, memory_type: str, new_content: str, memory_id: str=None):
        ...
        if memory_type == 'core':
            ...
            # For semantic and episodic memories, use the existing logic
            if not memory_id:
                raise ValueError("memory_id is required for updating semantic and episodic memories.")
            mem_list = getattr(self, memory_type)
