#!/bin/bash
# ==============================================================================
# EXTERNAL API MEMORY sweep — CURATOR method, executor=API AND curator=API.
# GPT-5.5 / GPT-5.4 / GPT-5.4-mini via the Salesforce gateway; Gemini-3.1-flash-lite via Vertex.
#
# Curator twin of server_api_memory_gpt_gemini.sh — SAME executor/curator hypers, but:
#   - memory_type = curator (uses run_unified_dev_async_curator_api.py + curator_alfworld_api.py)
#   - hist = 5 (this jul18 sweep; earlier runs were hist=3 — override via HIST=)
#   - adds the curator-specific axes from server_8b_8b_curator_jul13th.sh:
...
#
# Usage:
#   bash server_api_curator_gpt_gemini.sh --dry-run
#   tmux new -s api_cur 'bash .../server_api_curator_gpt_gemini.sh'   # 3 seeds in parallel
...
SWEEP_LOG="logs_debug_memory/_sweep_api_curator_$(date +%Y%m%d_%H%M%S).log"
...
RUNS="${RUNS:-1 2 3}"
HIST="${HIST:-5}"                  # hist=5 this sweep (earlier api-cur runs were hist=3)
...
# config axis in the exp-name (temp/hist/rn/tc/oe/run) — otherwise a folder from a DIFFERENT
# config (e.g. the earlier hist3 runs) would satisfy a hist5 job and wrongly skip it. Only the
# trailing STAMP is wildcarded.
already_done() {
  local model="$1" tc="$2" run="$3" d n
  local pat="dev_api-cur_*_temp${EXECUTOR_TEMPERATURE}_hist${HIST}_rn${RETRIEVE_NUM}_tc${tc}_oe${ON_EMPTY}_run${run}_*_few_shot_False_curator"
...
run_exp() {
  ...
  local exp="api-cur_${tag}_temp${EXECUTOR_TEMPERATURE}_hist${HIST}_rn${RETRIEVE_NUM}_tc${tc}_oe${ON_EMPTY}_run${run}_${STAMP}"
  ...
  if [ "$DRY_RUN" = 1 ]; then
    ...
    echo "  [${route}] curator exec=${model} curator=${model} hist=${HIST} rn=${RETRIEVE_NUM} tc=${tc} on_empty=${ON_EMPTY} run=${run} cur(max=4096) -> ${exp}"
    return 0
  fi
...
echo "[api-curator] models='${MODELS}'  hist=${HIST}  tc_list='${TC_LIST}'  on_empty=${ON_EMPTY}  runs='${RUNS}'"
