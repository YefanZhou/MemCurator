#!/bin/bash
# ==============================================================================
# EXTERNAL-SERVERS variant of server_8b_8b_curator_jul19th_extend_bothnonhink.sh.
# Use when the vLLM servers are ALREADY up (launched separately, e.g. launch_vllm_4port_dp2.sh
# -> :8001-:8004 dp=2). This script does NOT launch or tear down servers; it drains the 24-job
# queue ACROSS the given ports IN PARALLEL. Also BOTH-nonthink: executor AND curator nonthinking
# (CURATION_ENABLE_THINKING=false); folders named cur_bothnonthink_* (distinct from the older
# cur_nonthink_* runs whose curator was thinking).
#   bash ...bothnonhink_1port.sh --dry-run
#   tmux new -s cur1p 'bash ...bothnonhink_1port.sh'   # uses :8001-:8004
#   PORTS="8001" bash ...                              # single endpoint
#   PORTS="8001 8002" bash ...                         # subset
# ------------------------------------------------------------------------------
...
export CURATION_TEMPERATURE=0.6 CURATION_TOP_P=0.95 CURATION_TOP_K=20 CURATION_MAX_TOKENS=4096 CURATION_ENABLE_THINKING=false
...
# EXTERNAL-SERVERS variant: the vLLM servers are already up (launched separately, e.g. via
# launch_vllm_4port_dp2.sh -> :8001-:8004 dp=2 each). This script does NOT launch or tear down
# vLLM — it just drains the job queue across those ports in parallel. --no-serve is forced on.
# Default = the 4 ports from launch_vllm_4port_dp2.sh; override with PORTS="8001" for a single
# endpoint, or PORTS="8001 8002" for a subset.
...
BASE=(
  "false|5|cur_bothnonthink_hist5_run1"
  "false|5|cur_bothnonthink_hist5_run2"
  "false|5|cur_bothnonthink_hist5_run3"
  "false|3|cur_bothnonthink_hist3_run1"
  "false|3|cur_bothnonthink_hist3_run2"
  "false|3|cur_bothnonthink_hist3_run3"
)
...
QUEUE="logs_debug_memory/_curator_queue_8b8b_extend_1port.txt"
QLOCK="logs_debug_memory/_curator_queue_8b8b_extend_1port.lock"
...
if [ "$DRY_RUN" = 1 ]; then
  echo "===================== DRY RUN ====================="
  echo "SERVERS: using EXISTING endpoint(s) ${PORTS[*]} (no launch/teardown; NO_SERVE=${NO_SERVE})"
  echo "SHARED QUEUE (${#JOBS[@]} jobs) — ${#PORTS[@]} port(s) drain in parallel:"
