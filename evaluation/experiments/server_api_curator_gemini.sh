#!/bin/bash
# ==============================================================================
# EXTERNAL API MEMORY sweep — CURATOR method, GEMINI-ONLY (executor=API AND curator=API).
# Gemini-3.1-flash-lite via Vertex AI. (gpt-only twin: server_api_curator_gpt_gemini.sh)
#
...
#
# DEFAULT: gemini-3.1-flash-lite only, task_context=short, seeds {1,2,3}, rn=5, hist=5
# -> 3 jobs, ALL PARALLEL. Parallelism is at the JOB level (each seed is its own process);
# JOB_PARALLEL caps how many run at once (default = all). Each job also fans out MAX_CONCURRENCY
# concurrent Vertex calls.
#
# Usage:
#   bash server_api_curator_gemini.sh --dry-run
#   tmux new -s api_cur_gem 'bash .../server_api_curator_gemini.sh'   # 3 seeds in parallel
#   JOB_PARALLEL=2 bash ...                                       # cap to 2 concurrent seeds
#   NUM_GAMES=6 bash ...                                          # fast debug pass
#   TC_LIST="short obs0" bash ...                                # widen task_context
# ==============================================================================
set -u
...
SWEEP_LOG="logs_debug_memory/_sweep_api_curator_gemini_$(date +%Y%m%d_%H%M%S).log"
...
# What to sweep. Default: gemini-3.1-flash-lite only, task_context=short, seeds 1/2/3, rn=5, hist=3.
MODELS="${MODELS:-gemini/gemini-3.1-flash-lite}"
RETRIEVE_NUM="${RETRIEVE_NUM:-5}"
NUM_GAMES="${NUM_GAMES:-0}"        # 0 => all 140
RUNS="${RUNS:-1 2 3}"
HIST="${HIST:-5}"                  # hist=5 ONLY (per request)
...
# Already finished? (140-idx folder for this model/tc/run, any stamp).
already_done() {
  local model="$1" tc="$2" run="$3" d n
  for d in Alfworld/results/${model}/dev_api-cur_*_tc${tc}_*_run${run}_*_few_shot_False_curator; do
    [ -d "$d" ] || continue
