#!/bin/bash
# ==============================================================================
# NODE A: self-contained CURATOR sweep, jul-13. Qwen3-8B executor + Qwen3-8B
...
#
# MemCurator twin of the SkillOS sweep, plus curator-specific axes:
...
#         x curator_on_empty{ON} = 12 x 2 x 2 x 1 = 48 runs.
...
#
# Memory (curator_memory.jsonl + curator_calls.jsonl) lives INSIDE each result
# folder, so --overwrite wipes it along with the results.
#
# Usage:
#   bash server_8b_8b_curator_jul13th.sh --dry-run     # print servers + all jobs, run nothing
#   bash server_8b_8b_curator_jul13th.sh --no-serve    # skip server launch (servers already up)
#   NO_TEARDOWN=1 bash ...                             # leave vLLM servers running at the end
#   tmux new -s curA 'bash .../server_8b_8b_curator_jul13th.sh'
#   STAMP=jul13b bash .../server_8b_8b_curator_jul13th.sh
# ==============================================================================
set -u
...
SWEEP_LOG="logs_debug_memory/_sweep_curator_8b8b_$(date +%Y%m%d_%H%M%S).log"
...
BASE=(
  "true|5|cur_think_hist5_run1"
  "true|5|cur_think_hist5_run2"
  "true|5|cur_think_hist5_run3"
...
QUEUE="logs_debug_memory/_curator_queue_8b8b.txt"
QLOCK="logs_debug_memory/_curator_queue_8b8b.lock"
