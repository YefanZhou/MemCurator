#!/bin/bash
# ==============================================================================
# EXTERNAL API MEMORY sweep — CURATOR_V1 method, GPT-ONLY (jul-16).
# GPT-5.x via the Salesforce gateway. (Gemini variant: server_api_curator_v1_gemini_jul16th.sh.)
# Split out from server_api_curator_v1_gpt_gemini.sh so the gateway (gpt) lane runs
# independently of the Vertex (gemini) lane. Only the MODELS default differs.
#
...
#       faithful-prompt curator, --curation_mode {success_only, success_only_v1, success_and_fail,
#       success_and_fail_v1}, reward-aware store, full-prompt + BM25-provenance logging.
#       THIS SWEEP: --curation_mode success_only_v1 (the REVISED success-only briefing prompt,
#       CURATOR_SYSTEM_SUCCESS_ONLY_V1). Override via CURATION_MODE=... .
#   - hist = 3 AND 5 (both history windows; override via HIST_LIST=...)
...
#
# DEFAULT: gpt-5.4 only, task_context=short, seeds {1,2,3}, rn=5, hist=3 -> 3 jobs, ALL PARALLEL.
# Parallelism is at the JOB level (each seed is its own process); JOB_PARALLEL caps how many run
# at once (default = all). Each job also fans out MAX_CONCURRENCY concurrent gateway calls.
#
# Usage:
#   bash server_api_curator_v1_gpt_jul16th.sh --dry-run
#   tmux new -s api_curv1_gpt 'bash .../server_api_curator_v1_gpt_jul16th.sh'   # 3 seeds in parallel
#   JOB_PARALLEL=2 bash ...                                       # cap to 2 concurrent seeds
#   NUM_GAMES=6 bash ...                                          # fast debug pass
#   MODELS="openai/gpt-5.4 openai/gpt-5.4-mini" TC_LIST="short obs0" bash ...   # widen the sweep
# ==============================================================================
set -u
...
# What to sweep. Default: gpt-5.4 only, task_context=short, seeds 1/2/3, rn=5, hist=3.
MODELS="${MODELS:-openai/gpt-5.4}"
RETRIEVE_NUM="${RETRIEVE_NUM:-5}"
NUM_GAMES="${NUM_GAMES:-0}"        # 0 => all 140
RUNS="${RUNS:-1 2 3}"
HIST_LIST="${HIST_LIST:-3 5}"      # history windows to sweep (both hist=3 and hist=5)
...
# args: model task_context run hist
run_exp() {
  local model="$1" tc="$2" run="$3"
  local HIST="$4"                 # per-job history window (shadows global; already_done reads it)
...
# Build the flat job list of (model, tc, run, hist) tuples.
JOBS=()
for m in $MODELS; do for tc in $TC_LIST; do for h in $HIST_LIST; do for r in $RUNS; do JOBS+=("${m}|${tc}|${r}|${h}"); done; done; done; done
NJOBS=${#JOBS[@]}
[ "$JOB_PARALLEL" -le 0 ] 2>/dev/null && JOB_PARALLEL=$NJOBS   # 0/auto => all jobs at once

echo "[api-curator] models='${MODELS}'  hist_list='${HIST_LIST}'  tc_list='${TC_LIST}'  on_empty=${ON_EMPTY}  mode=${CURATION_MODE}  runs='${RUNS}'"
...
if [ "$DRY_RUN" = 1 ]; then
  ...
  n=0
  for m in $MODELS; do for tc in $TC_LIST; do for h in $HIST_LIST; do for r in $RUNS; do run_exp "$m" "$tc" "$r" "$h"; n=$((n+1)); done; done; done; done
...
# Track job PIDs and wait ONLY on them — a bare `wait` also waits on the `tee` from the
# `exec > >(tee ...)` process substitution, which never exits until this shell does, so a bare
# `wait` deadlocks (the script hangs after the last job DONE and never returns to a caller).
job_pids=()
running=0
for job in "${JOBS[@]}"; do
  IFS='|' read -r m tc r h <<< "$job"
  run_exp "$m" "$tc" "$r" "$h" &
  job_pids+=("$!")
  running=$((running+1))
  if [ "$running" -ge "$JOB_PARALLEL" ]; then
    wait -n "${job_pids[@]}" 2>/dev/null || wait -n 2>/dev/null   # free a slot when any job finishes
    running=$((running-1))
  fi
done
wait "${job_pids[@]}"   # only the jobs — NOT tee (avoids the process-substitution deadlock)
