#!/bin/bash
# ==============================================================================
# CROSS-BACKEND CURATOR_V1 sweep, jul-20 — GPT executor + LOCAL Qwen3-8B curator.
# Executor = gpt-5.x via the Salesforce gateway; CURATOR = Qwen3-8B (thinking) served
# LOCALLY on :8001-:8004 (dp=2 each). --curation_mode success_only_v1. Curator temp axis
# CT_LIST=(1.0 0.6).
#
# Derived from server_api_curator_v1_gpt_jul16th_extra_success_v1.sh, with the CURATOR
...
# The EXECUTOR still hits the gateway (LLM_BACKEND=openai, OPENAI_API_BASE=gateway); executor
# and curator base_urls are independent in the runner, so this cross-backend split works.
#
...
#
# DEFAULT: gpt-5.4 executor, tc=short, seeds {1,2,3}, rn=5, hist{3,5}, CT{1.0,0.6}
#          => 1 model x 1 tc x 2 hist x 3 seeds x 2 ct = 12 jobs.
#
# Usage:
#   bash server_api_curator_v1_gpt_jul20th_qwencur_successv1.sh --dry-run
#   tmux new -s apiqc_gpt 'bash .../server_api_curator_v1_gpt_jul20th_qwencur_successv1.sh'
#   CT_LIST="1.0" bash ...                                     # single curator temp
#   MODELS="openai/gpt-5.4 openai/gpt-5.4-mini" bash ...       # widen executors
# ==============================================================================
set -u
...
# ---- gateway config (EXECUTOR = openai/* via gateway) ----
...
# What to sweep. Executor = gpt-5.4; curator = local Qwen3-8B on the ports below.
