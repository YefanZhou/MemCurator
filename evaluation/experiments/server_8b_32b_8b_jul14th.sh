...
#
# 6 runs, all reasoningbank + thinking + revise_react, async:
...
#      HISTORY_LENGTH; ENABLE_THINKING=true fixed since all 6 jobs are thinking) ----
...
# ==============================================================================
# Job matrix — 6 reasoningbank thinking runs. Priority: hist5 first (headline), then hist3.
...
# prio: hist5 -> 0 (run first), hist3 -> 1. Stable sort keeps run1/2/3 order within a hist.
JOBS=()
for base in "${BASE[@]}"; do
...
if [ "$DRY_RUN" = 1 ]; then
  ...
  echo "SHARED QUEUE (${#JOBS[@]} jobs) — 3 exec lanes drain in parallel; hist5 first:"
