"""
run_unified_hyper_concurrent.py — FAST no-memory ALFWorld baseline.

Speed model (vs run_unified_hyper.py)
-------------------------------------
run_unified_hyper.py is STEP-synchronous: it fires one step for all N agents,
waits for the slowest, env.steps them together, then advances. The per-step
barrier + inter-step queue drain caps GPU utilisation (~40%).

This file is GROUP episode-async:
  - Games are processed in GROUPS of --batch_size.
  - Within a group, every game runs to COMPLETION concurrently in its own thread
    (its LLM call is OUTSIDE a shared PDDL lock, so episodes overlap on LLM latency).
  - A BARRIER at the end of each group waits for all games before the next group.
This keeps the vLLM request queue full for most of each group -> high GPU util,
while the group barrier gives a clean point for (future) per-group memory updates.
...
def run_one_game(game_file, game_idx, model, max_steps):
    ...
    for step_count in range(max_steps):
        ...
        if step_count == 0:
            with _PRINT_LOCK:
                print(f'\033[92m[game {game_idx}] step0 response:\n{_short(response)}\033[0m')
...
def main(args):
    ...
    # --- process games in groups; barrier at the end of each group ---
    for gstart in range(0, n, group_size):
        group = [(gi, game_files[gi])
                 for gi in range(gstart, min(gstart + group_size, n))
                 if gi not in done_idx]
        if not group:
            continue
        ...
        # --- group barrier reached here; (hook for future per-group memory update) ---

        # running accuracy over all files written so far
        total = 0
        rew = 0.0
        for f in os.listdir(output_path):
            if f.endswith('.json'):
        ...
        print(f'[group {gstart}-{gstart + len(group)}] finished. '
              f'Running accuracy: {rew / max(total, 1) * 100:.2f}%  ({int(rew)}/{total})')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Fast concurrent no-memory ALFWorld baseline (group episode-async)')
    ...
    parser.add_argument('--batch_size',  type=int, default=30,
                        help='Group size = max concurrent games in flight')
