...
# Console verbosity knobs (env-overridable):
#   PROMPT_SHOW_EVERY : every N steps, print the FULL prompt of the first active agent
#                       (0 = never). Lets us spot-check prompt correctness (e.g. task present).
# Per-step agent responses are printed for the FIRST active agent only (reduced I/O).
PROMPT_SHOW_EVERY = int(os.environ.get("PROMPT_SHOW_EVERY", "0") or "0")

# Executor sampling hyperparameters (env-overridable). Unset/empty -> default/server default.
def _env_float(name):
...
EXECUTOR_MAX_TOKENS  = _env_int("EXECUTOR_MAX_TOKENS")            # None -> uncapped
EXECUTOR_TEMPERATURE = _env_float("EXECUTOR_TEMPERATURE")
if EXECUTOR_TEMPERATURE is None:
    EXECUTOR_TEMPERATURE = 0.7                                   # preserve prior hardcoded default
EXECUTOR_TOP_P = _env_float("EXECUTOR_TOP_P")                    # None -> server default
EXECUTOR_TOP_K = _env_int("EXECUTOR_TOP_K")                      # None -> server default
...
You should first reason step-by-step about the current situation. This reasoning process MUST be enclosed within <reason> </reason> tags.
...
Now it's your turn to take an action.
You should first reason step-by-step about the current situation with the help of past relevant {context_label_lower}. This reasoning process MUST be enclosed within <reason> </reason> tags.
Once you've finished your reasoning, you should choose an admissible action for current step and MUST present it within <action> </action> tags.\
"""
...
You should first reason step-by-step about the current situation, then think carefully which admissible action best advances the shopping goal. This reasoning process MUST be enclosed within <reason> </reason> tags.
Once you've finished your reasoning, you should choose an admissible action for current step and present it within <action> </action> tags.
"""
...
def llm(prompt, stop=None, model="openai/Qwen/Qwen2.5-7B-Instruct"):
    ...
    # extra_body carries vLLM-specific fields: the no-think flag (always) + top_k (if set).
    _extra_body = {"chat_template_kwargs": {"enable_thinking": False}}
    if EXECUTOR_TOP_K is not None:
        _extra_body["top_k"] = EXECUTOR_TOP_K
    _kwargs = dict(
        model=model,
        messages=messages,
        ...
        stop=stop,
        # Disable Qwen3 thinking per-request (matches memory_server.py / verl reward_manager).
        # Requires the vLLM server to run the STOCK Qwen3 chat template (NOT qwen3_nothink.jinja).
        extra_body=_extra_body,
    )
    if EXECUTOR_MAX_TOKENS is not None:
        _kwargs["max_tokens"] = EXECUTOR_MAX_TOKENS
    if EXECUTOR_TOP_P is not None:
        _kwargs["top_p"] = EXECUTOR_TOP_P
    response = completion(**_kwargs)
...
def alfworld_run_batch(env, obs, names, task_descriptions, admissible_commands,
    ...
    for _step in range(max_steps):
        ...
        _probe_idx = active_tasks[0]   # the one agent we print each step
        ...
        for idx in active_tasks:
            ...
            # periodically show the FULL prompt of the probe agent, to spot-check correctness
            if idx == _probe_idx and PROMPT_SHOW_EVERY and (_step % PROMPT_SHOW_EVERY == 0):
                print(f'\033[96m[PROMPT step {_step} agent {idx}]\n{prompt_text}\033[0m')
        ...
        with ThreadPoolExecutor(max_workers=len(active_tasks)) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    # reduced I/O: only print the first active agent's response this step
                    if idx == _probe_idx:
...
def webshop_run_batch(manager, obs_dict, max_steps=30,
    ...
    for _step in range(max_steps):
        ...
        _probe_idx = active[0]   # the one agent we print each step
        ...
        with ThreadPoolExecutor(max_workers=len(active)) as executor:
            ...
            for i in active:
                ...
                if ctx_text and step_counts[i] > 0:
                    ...
                    content = prompts[i]
                if i == _probe_idx and PROMPT_SHOW_EVERY and (_step % PROMPT_SHOW_EVERY == 0):
                    print(f'\033[96m[PROMPT step {_step} agent {i}]\n{content}\033[0m')
...
def main(args):
    ...
    if args.env == 'alfworld':
        ...
        for idx in tqdm(range(math.ceil(num_games_to_run / env.batch_size))):
            ...
            # STEP-0 TASK BUG FIX: was [1:2], which kept only the room description and
            # dropped the "Your task is to: ..." line. The step-0 template has no
            # {task_description} field, so the model got NO goal on the first step. [1:]
            # keeps room + task. Affects ONLY step 0 — step>0 overwrites current_obs with
            # env.step()'s observation (never sliced), task injected via template field.
            ob_list           = ['\n'.join(ob.split('\n\n')[1:]) for ob in ob_list]
