...
Now it's your turn to take an action.
You should first reason step-by-step about the current situation with the help of past relevant guidelines. This reasoning process MUST be enclosed within <think> </think> tags.
Once you've finished your reasoning, you should choose an admissible action for current step and MUST present it within <action> </action> tags.
"""
...
def alfworld_run_batch(obs, names, task_descriptions, admissible_commands,
    ...
    for step in range(max_steps):
        ...
        #     print(f'\033[94mPrompt for task {idx}: \n{prompt[0]["content"]}\033[0m')
...
def main(args):
    ...
    # Memory init
    if args.use_memory:
        # Set env vars before importing memory/llm_api (which reads them at module load)
        if args.mem_model:
    ...
    for idx in tqdm(range(math.ceil(num_games / env.batch_size))):
        ...
        # Build retrieved skills per task; also track workflow/memory for update
        retrieved_skills_list = None
        workflow_list = []
        memory_list = []
        if args.use_memory and len(Pro_Mem.documents) > 0:
            ...
            for task_desc in task_descriptions:
                workflow = Pro_Mem.retrieve(task_desc)
                ...
                skills_text = json.dumps(workflow, indent=4, ensure_ascii=False)
                retrieved_skills_list.append(skills_text)
        else:
            workflow_list = [None] * len(ob_list)
            memory_list = [None] * len(ob_list)
        ...
        if args.use_memory and Pro_Mem.is_cold_start == False:
            ...
            Pro_Mem.update(task_descriptions, trajectory_list, reward_list, workflow_list, memory_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mem_model', type=str, default=None, help='Model for memory curation (overrides MODEL_NAME env var)')
    parser.add_argument('--mem_base_url', type=str, default=None, help='Base URL for memory curation model (overrides API_BASE_URL env var)')
    parser.add_argument('--overwrite', action='store_true')
    args = parser.parse_args()

    if args.overwrite:
        result_dir = f'Alfworld/results/{args.model}/{args.split}_{args.exp_name}_few_shot_{args.few_shot}_memory_{args.use_memory}'
