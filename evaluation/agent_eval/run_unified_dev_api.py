"""
run_unified_dev.py  —  Single entry-point for all MemP experiments.

Development copy of run_unified.py with the ALFWorld step-0 task bug fixed
(observation slice changed from [1:2] to [1:] so the task goal is present on the
first step). See bug_fix_markdown/STEP0_TASK_BUG.md and method_notes/ for details.
...
# When external, vLLM-only extra_body fields (top_k, enable_thinking)  #
# are NEVER sent (gateway rejects them). If X_API_KEY is set it is     #
# attached as the X-Api-Key header (Salesforce gateway).              #
# ------------------------------------------------------------------ #
...
print(f"[backend] executor={LLM_BACKEND} (external={_EXEC_EXTERNAL})  "
      ...
      f"x_api_key={'set' if _X_API_KEY else 'unset'}")
...
_TRACKED_ENV_VARS = [
    ...
    "LLM_BACKEND", "CURATION_LLM_BACKEND", "X_API_KEY",
]
...
def alfworld_run_batch(env, obs, names, task_descriptions, admissible_commands,
    ...
    for step_idx in range(max_steps):
        ...
        for idx in active_tasks:
            ...
            if done[idx]:
                task_rewards[idx] = float(won[idx])   # normalize bool -> 1.0/0.0 (matches async runner)
            else:
...
def _build_curation_messages_plain(skill_memory: SkillMemory,
    ...
    Mirrors the native branch of SkillOS/skills_agent.py (is_qwen==False)."""
    trajectory_text = trajectory_to_text(messages)
    result_str = "Success" if reward else "Failure"
    system_messages = skill_memory.render_system_prompt(status='memorie')
    user_content = f"""# Task Context
...
def curation_llm_native(messages: list, curation_model: str, curation_base_url: str):
    ...
    Returns parsed function calls as [{name, arguments(JSON str)}] (shape _apply_parsed_function_calls
    expects). Mirrors SkillOS/skills_agent.py:366-402."""
    kwargs = dict(
...
def batch_update_skills_from_trajectories(
    ...
    use_http = curation_base_url is not None or _CUR_EXTERNAL or (
        curation_model is not None and curation_model.startswith("gemini/")
    )

    if use_http and _CUR_EXTERNAL:
        # External gateway (GPT / Gemini): native OpenAI tool-calling. PLAIN messages + tools=,
        # parse message.tool_calls (no ✿-token encode/parse).
        all_messages = []
        ...
        with ThreadPoolExecutor(max_workers=len(valid_indices)) as executor:
            futures = {
                executor.submit(curation_llm_native, all_messages[i], curation_model, curation_base_url): i
...
def init_memory(args, storage_path, env_name='alfworld'):
    ...
    # _CUR_EXTERNAL (CURATION_LLM_BACKEND=openai) forces the HTTP/native path even with no
    # --curation_base_url (falls back to OPENAI_API_BASE = the gateway).
    ...
    # The external/native path (_CUR_EXTERNAL) uses plain messages + tools=, so skip the Qwen load.
    if args.memory_type == 'skillos' and not _CUR_EXTERNAL:
...
def main(args):
    ...
    if args.env == 'alfworld':
        ...
        for idx in tqdm(range(math.ceil(num_games_to_run / env.batch_size))):
            ...
            # Step-0 fix: keep room + task chunks ([1:]) instead of only the room ([1:2]).
            # The step-0 template (ALFWORLD_TEMPLATE_NO_HIS) has no {task_description}
            # field, so the task must ride along in the observation or the first action
            # is chosen blind. Only affects step 0; steps 1+ overwrite current_obs with
            # env.step()'s observation and inject the task via {task_description}.
            ob_list           = ['\n'.join(ob.split('\n\n')[1:]) for ob in ob_list]
