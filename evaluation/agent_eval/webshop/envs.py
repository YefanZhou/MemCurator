...
class WebshopWorker:
    ...
    def __init__(self, seed, env_kwargs):
        ...
        # Instantiate directly to bypass gym 0.26+ PassiveEnvChecker
        # (WebAgentTextEnv was designed for gym <0.26 and lacks action_space)
        self.env = WebAgentTextEnv(**env_kwargs)
    
    def _normalize_action(self, action: str) -> str:
        """Case-insensitive click resolution against current clickables.

        Models often output lowercase action targets (e.g. 'click[buy now]')
        even though the WebShop environment requires the exact case ('Buy Now').
        Match against the live clickable keys and substitute the canonical form.
        """
        from web_agent_site.engine.engine import parse_action
        action_name, action_arg = parse_action(action)
        if action_name == 'click' and action_arg is not None and \
                self.env.text_to_clickable is not None:
            lower_arg = action_arg.lower()
            for key in self.env.text_to_clickable:
    ...
    def reset(self, idx):
        ...
        info = dict(info or {})
        info['available_actions'] = self.env.get_available_actions()
        info['instruction_text'] = self.env.instruction_text
        info['won'] = False
        return obs, info
...
class WebshopMultiProcessEnv(gym.Env):
    ...
    def __init__(
        ...
        # Initialize Ray locally (force local cluster to avoid remote cluster discovery)
        ...
        runtime_env = {"env_vars": {"PYTHONPATH": webshop_dir}}
        env_worker = ray.remote(runtime_env=runtime_env, **resources_per_worker)(WebshopWorker)
        self._workers = []
    ...
    def reset(self, indices=None):
        if indices is not None:
            idx = np.repeat(np.array(indices), self.group_n).tolist()
        else:
