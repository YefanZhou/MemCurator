...
def alfworld_run_batch(env, obs, names, task_descriptions, admissible_commands,
    ...
    for _ in range(max_steps):
        ...
        for idx in active_tasks:
            ...
            if step_count <= 2:
                print(f"[PROBE game {idx} step {step_count}] task_in_prompt={task_descriptions[idx].strip() in prompt_text} | prompt={prompt_text!r}")
...
def main(args):
    ...
    if args.env == 'alfworld':
        ...
        for idx in tqdm(range(math.ceil(num_games_to_run / env.batch_size))):
            ...
            # FIX: [1:] keeps room description AND the task line (was [1:2], which dropped the task).
            ob_list           = ['\n'.join(ob.split('\n\n')[1:]) for ob in ob_list]
