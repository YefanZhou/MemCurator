...
from skills_agent import SkillAgent
from skills_memory import SkillMemory
from skills_functions import get_memory_tool_schemas
...
UNIFIED_PROMPT = "# Input (New Experience) \n\n{context}\n\n**Important**: Response limit is {max_new_tokens} tokens. Focus on **generalization**: extract the *insights* behind the solution, not just the solution itself. "
...
def run_streaming_sequential(args, agent_config, batch_queries_and_answers, trial):
    ...
    memory = SkillMemory()
    agent = SkillAgent(agent_config=agent_config)
    ...
    for i, item in enumerate(queries):
        ...
        raw_update_content = UNIFIED_PROMPT.format(
        ...
        processed_update_prompt = SkillAgent.process_text_with_qwen_pipeline(
        ...
        print(f"✅ 第 {i+1} 题完成. Correct: {is_correct}. 当前记忆条数: {len(memory.skills)}")
