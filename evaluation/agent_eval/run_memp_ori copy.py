...
# Respect an externally-set ALFWORLD_DATA; only fall back to the repo-relative default.
os.environ.setdefault("ALFWORLD_DATA", "../alfworld/datasets")
...
def alfworld_run_batch(obs:list=[],names:list=[],few_shot=True,max_steps=30,examples_list=[],model="gpt-4o"):
    ...
    for i in range(max_steps):
        ...
        with ThreadPoolExecutor(max_workers=len(active_tasks)) as executor:
            futures = {executor.submit(llm, messages_list[idx], model=model): idx for idx in active_tasks}
...
def main(args):
    ...
    for idx in tqdm(range(math.ceil(num_games/env.batch_size))):
        ...
        # get_prompt_list
        batch_results = alfworld_run_batch(obs=ob_list,names=name_list, few_shot=args.few_shot, max_steps=args.max_steps,examples_list=examples_list, model=model_name)
