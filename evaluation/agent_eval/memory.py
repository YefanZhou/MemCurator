...
from memory_adjust import adjust_memory
...
class Memory:
            ...
            if os.path.exists(self.documents_path):
                ...
                if self.documents:
                    self.bm25_retriever = BM25Retriever.from_documents(
                        self.documents, k=self.retrieve_num
                    )
    ...
    def _save_documents(self):
        """
        Save all current documents to disk and rebuild BM25 retriever.
        """
    ...
    def retrieve(self, key):
        """
        Retrieve from memory using BM25.
        Returns list of (Document, score) tuples for "query"/"facts" policies,
        or list of Documents for "random".
        """
        if self.bm25_retriever is None:
            return []

        if self.retrieve_policy in ["query", "facts", "ave_fact"]:
            docs = self.bm25_retriever.invoke(str(key))
