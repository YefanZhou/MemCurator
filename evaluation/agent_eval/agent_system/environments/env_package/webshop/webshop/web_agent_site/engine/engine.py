...
# Stub out pyserini.encode to avoid flash_attn / transformers import failure
import sys as _sys
import types as _types
if 'pyserini.encode' not in _sys.modules:
    _enc = _types.ModuleType('pyserini.encode')
    for _n in ['QueryEncoder', 'CachedDataQueryEncoder', 'SlimQueryEncoder', 'SpladeQueryEncoder',
               'TokFreqQueryEncoder', 'UniCoilQueryEncoder', 'AnceQueryEncoder',
               'AutoQueryEncoder', 'AggretrieverQueryEncoder', 'DocumentEncoder',
               'AnceDocumentEncoder', 'TctColBertDocumentEncoder', 'AggretrieverDocumentEncoder']:
        setattr(_enc, _n, None)
    _sys.modules['pyserini.encode'] = _enc
del _sys, _types, _enc, _n
import os as _os; _os.environ.setdefault('JAVA_HOME', '/usr/lib/jvm/temurin-21-jdk-amd64'); del _os
