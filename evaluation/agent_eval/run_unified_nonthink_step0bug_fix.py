...
def main(args):
    ...
    if args.env == 'alfworld':
        ...
        for idx in tqdm(range(math.ceil(num_games_to_run / env.batch_size))):
            ...
            # NO goal on the first step. [1:] keeps room + task. Only affects step 0;
            # steps 1+ overwrite current_obs with the env's next observation and inject the
            # task via the template's {task_description} field.
            ob_list           = ['\n'.join(ob.split('\n\n')[1:]) for ob in ob_list]
