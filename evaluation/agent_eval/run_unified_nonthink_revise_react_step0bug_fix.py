...
Now it's your turn to take an action.
You should first reason step-by-step about the current situation with the help of past relevant {context_label_lower}.
Once you've finished your reasoning, you should choose an admissible action for current step and MUST present it within <action> </action> tags.\
"""
...
def main(args):
    ...
    if args.env == 'alfworld':
        ...
        for idx in tqdm(range(math.ceil(num_games_to_run / env.batch_size))):
            ...
            # dropped the "Your task is to: ..." line. The step-0 template
            # (ALFWORLD_TEMPLATE_NO_HIS) has no {task_description} field, so the model got
            # NO goal on the first step. [1:] keeps room + task. Affects ONLY step 0 — at
            # step>0 current_obs is overwritten with env.step()'s observation (never sliced),
            # and the task is injected via the template's {task_description} field.
            ob_list           = ['\n'.join(ob.split('\n\n')[1:]) for ob in ob_list]
