...
#   EXECUTOR_TEMPERATURE  (float, default 0.7)                        #
#   EXECUTOR_TOP_P        (float, default unset -> server default)    #
#   EXECUTOR_TOP_K        (int,   default unset -> server default;    #
#                          sent via extra_body, vLLM-specific)        #
#   EXECUTOR_MAX_TOKENS   (int,   default unset -> uncapped)          #
# Only variables that are set are forwarded to the request.           #
# ------------------------------------------------------------------ #
def _env_float(name):
...
print(f"[executor hyperparams] temperature={EXECUTOR_TEMPERATURE} "
      f"top_p={EXECUTOR_TOP_P} top_k={EXECUTOR_TOP_K} max_tokens={EXECUTOR_MAX_TOKENS}")
...
def llm(prompt, stop=None, model="openai/Qwen/Qwen2.5-7B-Instruct"):
    ...
    # Build sampling kwargs from env-configurable hyperparameters.
    # Standard OpenAI fields go top-level; top_k is vLLM-specific -> extra_body.
    kwargs = dict(
        model=model,
        messages=messages,
    ...
    if EXECUTOR_TOP_K is not None:
        kwargs["extra_body"] = {"top_k": EXECUTOR_TOP_K}
        ...
        with ThreadPoolExecutor(max_workers=len(active_tasks)) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    # Option A: only print the first active agent this step (cuts I/O ~batch_size×)
