...
class MemoryProcessor:
    ...
    def __init__(self, server_url=None):
        ...
        if self.model.startswith("qwen"):
            ...
            self.client_32b = OpenAI(
                base_url="http://0.0.0.0:8002/v1",
                api_key=os.getenv("OPENROUTER_API_KEY", "EMPTY")
            )

            self.tokenizer_32b = AutoTokenizer.from_pretrained("Qwen/Qwen3-32B")
            self.model_name_32b = "Qwen/Qwen3-32B"
    ...
    def _get_analysis_prompt(self, memory_type: str) -> str:
        ...
        if memory_type == 'core':
            return """You are an expert memory analyst. Analyze the quality of planning memory content.

The planning memory is invalid if any of the following meets:
(1) The literal content "planning memory" appears in the memory such as "This is planning memory ...", "The planning memory has been updated ...".
(2) The planning memory is apparently a placeholder such as "Here we save the summary" while not stating what the "summary" is, "Here are some rules" and not stating what the "rules" are.

A valid planning memory should contain the high-level meta reasoning protocals and strategies of problem-solving planning.
...
```"""
            ...
            return """You are an expert memory analyst. Analyze the quality of knowledge memory content.

Knowledge memory should contain:
- Information or Knowledge about mathematics
- Definitions, theories, principles, or explanations
- Research findings or established facts

Two other memories are Planning memory (Meta-Reasoning Protocols/Strategic Planning) and Skills memory (Concrete Strategies). The information not suitable for these two memories should be considered as knowledge memory.
...
```"""
            ...
            return """You are an expert memory analyst. Analyze the quality of skills memory content.

skills memory should contain:
- Concrete Strategies
- Reusable skills, concepts, or insights
...
```"""
    ...
    def _format_memory_block(self, memory_list: List[Dict[str, str]], block_name: str) -> str:
        ...
        if block_name == "semantic_memory":
            block_title = "Knowledge Memory"
        elif block_name == "episodic_memory":
            block_title = "Skills Memory"
        elif block_name == "skills_memory":
            block_title = "Skills"
        ...
        for i, memory_item in enumerate(memory_list):
            if isinstance(memory_item, dict):
                ...
                if len(memory_item) == 1:
                    ...
                    title, content = list(memory_item.items())[0]
                    
                else:
                    # Multiple keys, look for common content keys
                    title, content = memory_item.get('title'), memory_item.get('content', str(memory_item))
                # key, value = list(memory_item.items())[0]
                # content = f"[{key}] {value}"
            else:
                content = str(memory_item)

            # formatted_memories.append(f"[Item-{i}]. {content}")
            formatted_memories.append(f"**Skill-{i}: {title}**\n{content}")
    ...
    def search_memories(self, memory_data: Dict[str, Any], query: str, top_k: int = 20) -> Dict[str, Any]:
        ...
        for memory_type in ['semantic', 'episodic', 'skills']:
            ...
            valid_documents = [] # 新增：用于存放分词后不为空的文档
            
            for i, content in enumerate(doc_contents):
                doc_tokens = self._tokenize(content)
                if doc_tokens: # 核心修复：只有分词结果不为空才加入
            ...
            scored_results = []
            for i, (memory_id, content, orig_idx) in enumerate(valid_documents):
                score = doc_scores[i]
                scored_results.append(({memory_id: content}, score, orig_idx))
    ...
    def construct_system_prompt(self, memory_data: Dict[str, Any], original_memory_data: Dict[str, Any] = None) -> str:
        ...
        skills_memories = memory_data.get('skills', [])  # list of dicts
        ...
        if core_memory and core_memory.strip():
            ...
            memory_structure_items.append("- Planning Memory: High-level Meta-Reasoning Protocols and Strategic Plannings")
        ...
        skills_block = self._format_memory_block(skills_memories, "skills_memory")

        # memory_blocks.extend([semantic_block, episodic_block, skills_block])
        memory_blocks.extend([skills_block])
        memory_structure_items.extend([
            "- Knowledge Memory: General and principled knowledge, definitional or conceptual information",
            "- Skills Memory: Specific, concrete strategies, useful insights, reusable skills/concepts"
        ])

        memory_structure = "\n".join(memory_structure_items)
        memory_content = "\n\n".join(memory_blocks)

#         system_prompt = f"""You are a reasoning assistant with access to structured memory. Use the memories below to provide correct responses to user queries.
...
# ### INSTRUCTIONS:
# 1. **Use the memories above to inform your responses**: Before reasoning, scan the provided Planning, Knowledge, and Skills memories.
# 2. **Memory Usage Statement**: 
#    - If you found some memories useful and helpful, you MUST explicitly reference them by mentioning their IDs or content.
#    - If no memory is helpful, you MUST explicitly state: "No helpful memory items found because..." followed by a brief reason.
# 3. **Reasoning**: Provide a step-by-step mathematical derivation and mentioning the available memory items.
# 4. **Consistency**: Ensure the final answer is derived logically from your reasoning.

# ### CONSTRAINTS:
# - Do not ignore the Memory Usage Statement; it is as important as the final answer.
# - If a memory item is used, you MUST **reference its specific insight** within the reasoning steps. Your answer must be grounded in the memories. Do not fabricate information.
# - Be concise but mathematically rigorous.
# """

#         system_prompt = f"""You are an expert with access to a list of skills. Use the skills below to provide correct responses to user queries.
...
# ### INSTRUCTIONS:
# 1. **Use the skills above to inform your responses**: Before reasoning, scan the provided skills.
# 2. **Skill Usage Statement**: 
#    - If you found some skills useful and helpful, you MUST explicitly reference them by mentioning their skill names or content.
#    - If no skill is helpful, you MUST explicitly state: "No helpful skill items found because..." followed by a brief reason. In this case, use your original knowledge and reasoning abilities to answer the question.
# 3. **Reasoning**: Provide a step-by-step reasoning process.
# 4. **Consistency**: Ensure the final answer is derived logically from your reasoning.

# ### CONSTRAINTS:
# - Do not ignore the Skill Usage Statement; it is as important as the final answer.
# - If a skill item is used, you MUST **reference its specific insight** within the reasoning steps.
# - Be concise but rigorous.
# """

        system_prompt = f"""You are an expert agent operating in the ALFRED Embodied Environment. 
Below are some skill items that I accumulated from past interaction from the environment that may be helpful to solve the task. You can use it when you feel it's relevant. In each step, please first explicitly discuss if you want to use each memory item or not, and then take action."
...
# - If some memories are relevant, you MUST reference them appropriately by mentioning their IDs or content
...
# """
        ...
        return system_prompt, memory_content
    ...
    def generate_response(self, memory_data: Dict[str, List], question: str) -> str:
        ...
        try:
            ...
            system_prompt, memory_content = self.construct_system_prompt(memory_data, memory_data)
            ...
            if token_count > 2048 * 15:
                ...
                system_prompt, memory_content = self.construct_system_prompt(filtered_memory_data, memory_data)
                token_count = self.count_tokens(system_prompt)
    ...
    try:
        ...
        for i, item in enumerate(all_items):
            try:
                ...
                if tool_name not in ['new_memory_insert', 'memory_update', 'skill_update', 'new_skill_insert']:
                    item_analyses[i] = {
                ...
                if tool_name == 'new_memory_insert':
                    ...
                    content = tool_arguments.get('new_title', '') + tool_arguments.get('new_content', '')
                    memory_type = "skills"  # Force memory type to skills for skill updates
                elif tool_name == 'new_skill_insert':
                    content = tool_arguments.get('content', '')
                    memory_type = "skills"  # Force memory type to skills for new skill inserts
                else:
                    item_analyses[i] = {
                ...
                if memory_type not in ['core', 'semantic', 'episodic', "skills", "knowledge"]:
                    item_analyses[i] = {
        ...
        for item_index, tool_name, tool_arguments, memory_type, content in valid_items:
            ...
            if processor.tokenizer is not None:
                prompt = processor.tokenizer_32b.apply_chat_template(
                    dict_messages,
                    tokenize=False,
        ...
        logger.info("initialized openai client for analysis")

        # Perform batch inference if we have valid items
        llm_results = []
        if all_prompts:
            if processor.model.startswith("qwen") and processor.tokenizer is not None:
                ...
                if len(all_prompts) > effective_batch_size:
                    ...
                    for i in range(0, len(all_prompts), effective_batch_size):
                        ...
                        try:
                            ...
                            resp = processor.client_32b.completions.create(
                                model=processor.model_name_32b,
...
@app.route('/batch_select_file_memory', methods=['POST'])
def batch_select_file_memory():
    """
    Select relevant files in a file-based memory system based on a memory tree view and a question.
    ...
    try:
        ...
        memory_tree_view = data.get('memory_tree_view', [])
        questions = data.get('questions', [])
        max_tokens = data.get('max_tokens', 2048)
        temperature = data.get('temperature', 0.7)
        ...
        if not memory_tree_view or not questions:
            return jsonify({"error": "'memory_tree_view' and 'question' are required"}), 400
        if len(memory_tree_view) != len(questions):
            return jsonify({"error": "Number of memory tree views must match number of questions"}), 400

        all_prompts = []
        structure_info = []

        for idx, (tree_view_str, question_list) in enumerate(zip(memory_tree_view, questions)):
            for q_idx, question in enumerate(question_list):

                system_prompt = """You are an expert File System Navigator and Memory Selection Specialist. Your task is to analyze the provided memory tree structure and identify the relevant `.md` files that can help answer the user's question.

### Context: File Structure
You will be provided with a `Memory Tree Structure`. The first line of this structure represents the **Root Directory Name**. All files and subdirectories are nested beneath this root.

# Constraints
- **Full Path Requirement**: Every path in your output MUST be a complete path starting with the Root Directory Name (e.g., "root_name/folder/file.md").
- **Strict JSON Output**: Return ONLY a valid JSON list of strings. No conversational text, no explanations, and no markdown code blocks.
- **Path Integrity**: Casing, slashes, and spelling must match the provided tree structure exactly.
- **File Type**: Select only files ending in `.md`.
- **Zero-Match Case**: If no relevant files exist, return `[]`.

# Output Format Example
["RootName/category/subcategory/target_file.md", "RootName/essentials/logic.md"]
    """
                
                user_instruction = """Use your expertise to identify relevant files given the following memory structure and user question.
...
Analyze the Question against the Memory Tree Structure provided above. Identify the full paths of all relevant `.md` files that likely contain the information needed to answer the question. 
"""
                
                dict_messages = [
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_instruction.format(tree_view_str=tree_view_str, question=question)}
                ]
                ...
                structure_info.append((idx, q_idx))
        ...
        if processor.model.startswith("qwen") and processor.tokenizer is not None:
            ...
            if len(all_prompts) > batch_size:
                ...
                logger.info(f"processing file selection prompt: {all_prompts}")

                all_results = [choice.text for choice in resp.choices]
                logger.info(f"Completed single batch with {all_results}")

        # Reshape results according to structure_info
        results = []
        ...
        for mem_idx, result_list in enumerate(results):
            for q_idx, result_str in enumerate(result_list):
                try:
                    selected_paths = json.loads(result_str)
                    if not isinstance(selected_paths, list):
                        raise ValueError("Result is not a list")
                    # Further validate each path is a string
                    for p in selected_paths:
                        if not isinstance(p, str):
                            raise ValueError("Path is not a string")
                except Exception as e:
                    logger.error(f"Error parsing selected paths for memory {mem_idx}, question {q_idx}: {str(e)}")
                    selected_paths = []

                results[mem_idx][q_idx] = selected_paths
        ...
        logger.error(f"Error selecting file memories: {str(e)}")
        return jsonify({"error": str(e)}), 500
    


@app.route('/file_batch_process', methods=['POST'])
def file_batch_process():

    """
    Batch processing for file-based memory system. Includes mini-batch processing to handle large batches efficiently and avoid API rate limits.
    ...
    try:
        ...
        if len(memories) != len(questions):
            return jsonify({"error": "Number of memories must match number of question sets"}), 400

        all_prompts = []
        structure_info = []

        for idx, (memory_content, question_list) in enumerate(zip(memories, questions)):
            for q_idx, question in enumerate(question_list):

                system_prompt = """You are a reasoning assistant with access to memories. Your goal is to solve math problems by integrating your internal knowledge with the provided external memories.
You will be provided with the content of several markdown (.md) files retrieved from a file-based memory system. Use this information to help answer the user's question.

### MEMORY Content:
{memory_content}

### MANDATORY RESPONSE PROTOCOL:
1. **Analyze Relevant Memories**: Before reasoning, scan the provided memory contents.
2. **Memory Usage Statement**: 
- If you use specific memories, you MUST explicitly list their file names (e.g., "./memories/skills/logic.md").
- If no memory is helpful, you MUST explicitly state: "No helpful memory items found because..." followed by a brief reason.
3. **Reasoning**: Provide a step-by-step mathematical derivation.
4. **Consistency**: Ensure the final answer is derived logically from your reasoning.

### CONSTRAINTS:
- Do not ignore the Memory Usage Statement; it is as important as the final answer.
- If a memory is used, reference its specific insight within the reasoning steps.
- Be concise but mathematically rigorous.
"""
                
                dict_messages = [
                    {"role": "system", "content": system_prompt.format(memory_content=memory_content)},
                    {"role": "user", "content": question}
                ]
        ...
        for mem_idx, result_list in enumerate(results):
            for q_idx, result_str in enumerate(result_list):
                assert result_str is not None, f"Missing result for memory {mem_idx}, question {q_idx}"
...
def batch_process():
    ...
    for mem_idx, (memory_data, question_list) in enumerate(zip(memories, questions)):
        for q_idx, question in enumerate(question_list):
            ...
            if "**QUESTION**" in question:
                search_memory_query = question.split("**QUESTION**:")[1].split("\n\n")[0].strip()
            elif "Your task is to:" in question:
                search_memory_query = question.split("Your task is to:")[1].split("\n")[0].strip()
            filtered_memory_data = processor.search_memories(memory_data, search_memory_query, top_k=top_k)

            # Construct system prompt with filtered memories
            system_prompt, memory_content = processor.construct_system_prompt(filtered_memory_data, memory_data)

            retrieved_skills.append(memory_content)
            ...
            # Create message dictionary
            # user_message = question.format(retrieved_skills=memory_content)
            user_message = question.replace("{retrieved_skills}", str(memory_content))
            dict_messages = [
                # {"role": "system", "content": system_prompt},
    ...
    for question_list in questions:
        ...
        mapped_memories.append([None] * len(question_list))  # For potential future use if needed

    # Place results in correct positions
    for result_idx, (mem_idx, q_idx) in enumerate(structure_info):
        results[mem_idx][q_idx] = all_results[result_idx]
        mapped_memories[mem_idx][q_idx] = retrieved_skills[result_idx]
    ...
    for mem_idx, result_list in enumerate(results):
        for q_idx, result in enumerate(result_list):
            ...
            assert mapped_memories[mem_idx][q_idx] is not None, f"Missing mapped memory for memory {mem_idx}, question {q_idx}"

    logger.info(f"Successfully processed {len(all_results)} completions")

    return jsonify({
        "result": results,
        "retrieved_skills": mapped_memories,
        "status": "success",
        "processed_count": len(all_results)
    })
...
if __name__ == '__main__':
    ...
    if not MODEL_NAME.startswith("qwen") and not os.getenv("AZURE_OPENAI_API_KEY"):
