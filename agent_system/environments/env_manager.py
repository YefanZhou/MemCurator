...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def step(self, text_actions: List[str]):
        ...
        # print(f"[DEBUG] actions parsed: {actions}")
    ...
    def build_text_obs(self, text_obs: List[str], admissible_actions: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            if init or self.config.env.history_length <= 0:
                ...
                obs = ALFWORLD_TEMPLATE_WITH_SKILLS.format(
                    task_description=self.tasks[i],
                    step_count=len(self.memory[i]),
    ...
    def get_task_type_for_slot(self, slot_idx: int):
        gamefile = self.gamefile[slot_idx]
        if gamefile is None:
            return None
    ...
    def lock_slot_task_types(self):
        ...
        self.reset({})
        slot_task_types = [self.get_task_type_for_slot(i) for i in range(len(self.gamefile))]
        print(f"[DEBUG] Locking slot task types: {set(t for t in slot_task_types if t)}")
        self.set_slot_task_types(slot_task_types)
