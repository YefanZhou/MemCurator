...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        # Lists to collect samples for dumping
        sample_inputs = []
        sample_outputs = []
        sample_scores = []
        sample_memories = []
        ...
        gen_config = MemoryGenerationConfig(
            ...
            respond_url=self.config.respond_url,
            analyze_function_url=self.config.analyze_function_url,
            enable_thinking=self.config.enable_thinking,
        )
        generation_manager = MemoryGenerationManager(
        ...
        for batch_idx, batch_dict in enumerate(self.val_dataloader):
            ...
            print(f"Processing Batch {batch_idx}/{total_batch}...")

            print("Data source: ", batch_dict['data_source'])
            ...
            with marked_timer('step', timing_raw):
                with marked_timer('gen', timing_raw):
                    ...
                    # For memory mode validation, process questions and answers
                    final_gen_batch_output = generation_manager.run_memory_loop(
                        gen_batch=test_gen_batch,
                        questions_and_answers=test_batch.non_tensor_batch.get('questions_and_answers', []),
                ...
                # use indices_in_batch to repeat the non_tensor_batch
                for key in test_batch.non_tensor_batch.keys():
                    new_items = []
                    current_val = test_batch.non_tensor_batch[key]

                    # Ensure the value is a numpy array for proper indexing
                    if not isinstance(current_val, np.ndarray):
                        current_val = np.array(current_val, dtype=object)

                    for i in indices_in_batch:
                        new_items.append(current_val[i])
                    test_batch.non_tensor_batch[key] = np.array(new_items, dtype=object)

                final_gen_batch_output.non_tensor_batch = test_batch.non_tensor_batch.copy()
                ...
                reward_tensor, acc_scores, compression_ratio_reward_scores, all_function_call_rewards, all_function_call_content_rewards, memory_indicator_scores = self.val_reward_fn(final_gen_batch_output, data_sources)
                ...
                # Store generated outputs and scores for dumping
                output_ids = final_gen_batch_output.batch["responses"]
                output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids]
                scores = reward_tensor.sum(-1).cpu().tolist()

                # Store original inputs for dumping
                input_ids = final_gen_batch_output.batch["prompts"]
                ...
                # expanded_compression_ratio_reward_scores = []
                # for i in indices_in_batch:
                #     expanded_compression_ratio_reward_scores.append(compression_ratio_reward_scores[i])

                # Store extra reward info for dumping
                ...
                # for i in final_gen_batch_output.meta_info['indices_in_batch']:
                #     flattened_data_sources.append(data_sources[i])
        ...
        for i in range(compression_ratio_reward_scores.shape[0]):
            data_source = data_sources[i]
            if data_source not in data_source_compression_ratio_reward_scores:
                data_source_compression_ratio_reward_scores[data_source] = []
            data_source_compression_ratio_reward_scores[data_source].append(compression_ratio_reward_scores[i].item())
        ...
        for i in range(all_function_call_rewards.shape[0]):
            data_source = data_sources[i]
            if data_source not in data_source_all_function_call_rewards:
                data_source_all_function_call_rewards[data_source] = []
            data_source_all_function_call_rewards[data_source].append(all_function_call_rewards[i].item())
        ...
        for i in range(all_function_call_content_rewards.shape[0]):
            data_source = data_sources[i]
            if data_source not in data_source_all_function_call_content_rewards:
                data_source_all_function_call_content_rewards[data_source] = []
            data_source_all_function_call_content_rewards[data_source].append(all_function_call_content_rewards[i].item())
        ...
        for i in range(memory_indicator_scores.shape[0]):
            data_source = data_sources[i]
            if data_source not in data_source_memory_indicator_scores:
                data_source_memory_indicator_scores[data_source] = []
            data_source_memory_indicator_scores[data_source].append(memory_indicator_scores[i].item())
        ...
        if val_data_dir:
            ...
            print(f"Dumped {len(sample_inputs)} validation samples to {val_data_dir}")

        # Log validation generations if enabled
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix="global_seqlen"):
        ...
        global_partition_lst = get_seqlen_balanced_partitions(global_seqlen_lst, k_partitions=world_size, equal_size=False)
        # reorder based on index. The data will be automatically equally partitioned by dispatch function
        global_idx = torch.tensor([j for partition in global_partition_lst for j in partition])

        batch.reorder(global_idx)
        for key in batch.meta_info.keys():
            if len(batch.meta_info[key]) == len(global_idx):
                batch.meta_info[key] = [batch.meta_info[key][i] for i in global_idx]
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            ...
                            questions_and_answers = batch.non_tensor_batch.get('questions_and_answers', [])

                            gen_batch = gen_batch.repeat(repeat_times=self.config.customized_grpo_rollout_n, interleave=True)

                            if self.config.sub_sample_question_ratio is not None and self.config.sub_sample_question_ratio < 1:
                                sampled_questions_and_answers = []
                                for question_and_answer in questions_and_answers:
                                    if len(question_and_answer) > 50:
                                        sampled_question_and_answer = random.sample(list(question_and_answer), int(len(question_and_answer) * self.config.sub_sample_question_ratio))
                                        sampled_questions_and_answers.append(sampled_question_and_answer)
                                    else:
                                        sampled_questions_and_answers.append(question_and_answer)
                                questions_and_answers = sampled_questions_and_answers

                            new_questions_and_answers = []
                            for question_and_answer in questions_and_answers:
                                new_questions_and_answers.extend([question_and_answer] * self.config.customized_grpo_rollout_n)
                            ...
                            for data_source in data_sources:
                                new_data_sources.extend([data_source] * self.config.customized_grpo_rollout_n)
                            data_sources = new_data_sources

                            print("length of data_sources: ", len(data_sources))
                            print("length of questions_and_answers: ", len(questions_and_answers))
                            print("length of gen_batch: ", len(gen_batch))
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        raise NotImplementedError("REMAX is not supported for memory mode")
                    ...
                    batch = batch.repeat(repeat_times=self.config.customized_grpo_rollout_n, interleave=True)
                    ...
                    for key in batch.non_tensor_batch.keys():
                        new_items = []
                        current_val = batch.non_tensor_batch[key]
                        # Ensure the value is a numpy array for proper indexing
                        if not isinstance(current_val, np.ndarray):
                            current_val = np.array(current_val, dtype=object)
                        for i in indices_in_batch:
                            new_items.append(current_val[i])
                        batch.non_tensor_batch[key] = np.array(new_items, dtype=object)
                    # batch = batch.union(final_gen_batch_output)
                    final_gen_batch_output.non_tensor_batch = batch.non_tensor_batch.copy()
                    batch = final_gen_batch_output

                    outputs = self.tokenizer.batch_decode(final_gen_batch_output.batch['responses'], skip_special_tokens=True)
                    for i in range(len(final_gen_batch_output.meta_info['every_chunk_length'])):
                        # Use add_special_tokens=False to avoid adding BOS/EOS when re-encoding
                        decoded_length = len(self.tokenizer(outputs[i], add_special_tokens=False)['input_ids'])
                        expected_length = final_gen_batch_output.meta_info['every_chunk_length'][i]
                        if not decoded_length == expected_length:
                            print(f"[DEBUG] length mismatch for index {i}: decoded={decoded_length}, expected={expected_length}")
                            print(f"[DEBUG] Original response tokens: {final_gen_batch_output.batch['responses'][i]}...")  # Show first 50 tokens
                            print(f"[DEBUG] Decoded text (first 200 chars): {outputs[i]}")
                            print(f"[DEBUG] Re-encoded tokens (with special): {self.tokenizer(outputs[i], add_special_tokens=True)['input_ids']}...")
                            print(f"[DEBUG] Re-encoded tokens (without special): {self.tokenizer(outputs[i], add_special_tokens=False)['input_ids']}...")
                            raise ValueError(f"length of response is not correct: decoded={decoded_length}, expected={expected_length}")
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_extra_infos_dict = compute_reward(batch, self.reward_fn, data_sources=data_sources)
                    ...
                    if reward_extra_infos_dict is not None:
                        ...
                        for ds in unique_data_sources:
                            ds_indices = np.where(np.array(data_sources) == ds)
                            acc_scores = acc_reward_scores[ds_indices]
                            compression_scores = compression_ratio_reward_scores[ds_indices]
                            function_call_scores = function_call_rewards[ds_indices]
                            function_call_content_scores = function_call_content_rewards[ds_indices]
                            total_scores = training_rewards[ds_indices]
                            memory_scores = memory_indicator_scores[ds_indices]

                            metrics[f'train/acc/{ds}'] = float(np.mean(acc_scores))
                            metrics[f'train/total_reward/{ds}'] = float(np.mean(total_scores))
                            metrics[f'train/compression_ratio_reward/{ds}'] = float(np.mean(compression_scores))
                            metrics[f'train/function_call_rewards/{ds}'] = float(np.mean(function_call_scores))
                            metrics[f'train/function_call_content_rewards/{ds}'] = float(np.mean(function_call_content_scores))
                            metrics[f'train/memory_indicator_scores/{ds}'] = float(np.mean(memory_scores))
                    ...
                    # outputs = self.tokenizer.batch_decode(final_gen_batch_output.batch['responses'], skip_special_tokens=True)
                    # for i in range(len(final_gen_batch_output.meta_info['every_chunk_length'])):
                    #     if not len(self.tokenizer(outputs[i])['input_ids']) == final_gen_batch_output.meta_info['every_chunk_length'][i]:
                    #         print("[DEBUG] length of response is not correct for index: ", i)
                    #         raise ValueError("length of response is not correct")
                    #     if not reward_tensor[i].sum() == final_gen_batch_output.meta_info['every_chunk_length'][i]:
                    #         print("[DEBUG] reward is not correct for index: ", i)
                    #         raise ValueError("reward is not correct")
