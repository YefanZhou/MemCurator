...
#
# ===================================================================================================
# ABLATION: NON-THINKING CURATOR + retrieve_num=5 (Qwen3-8B executor).
#   Copy of train_memcurator_full_direct_nonthink.sh with ONE further change:
#   RETRIEVE_NUM 3 -> 5 (L96 below). Stacks TWO ablations on the Qwen3-8B executor:
#     (a) non-thinking curator (enable_thinking=false, inherited from the nonthink launcher), and
#     (b) top-5 retrieval instead of top-3.
#   retrieve_num is a SINGLE knob applied to BOTH train and val (memcurator/generation.py L192
#   train S_T store + L511 val per-lane store; there is NO separate val_retrieve_num). So the
#   curator sees 5 retrieved memories in training AND validation.
#
#   EVAL PARITY (IMPORTANT): the final eval (run_unified_dev_async_curator.py --memory_type curator)
#   has its OWN --retrieve_num. To compare fairly, EVAL THIS CKPT WITH retrieve_num=5 too (and the
#   curator served non-thinking). Do NOT compare its val curve to the retrieve_num=3 / thinking
#   baselines — it is its own baseline (two ablations stacked).
#
#   Cost note: top-5 grows the curator input prompt (more retrieved trajectories -> more tokens).
#   max_prompt_length=32768 is generous (no overflow), but the curator rollout is a bit slower and
#   briefings may be longer.
# ===================================================================================================
#
...
exp_name="${EXP_NAME:-memcurator-FULL-qwen3-8b-alfworld-nonthink-retrieve5-iter1}"
...
# ---- MemCurator knobs ----
DATASET_PATH="${DATASET_PATH:-/fsx/sfr/yefan.zhou/mem-evolve/data/datasets/full_dataset_hist3_frac0.5_iter1/dataset.jsonl}"
...
# ABLATION: top-5 retrieval (was 3). SINGLE knob -> applies to BOTH train (generation.py L192)
# and val (L511). To compare fairly, eval this ckpt with --retrieve_num 5 too.
...
VAL_EXECUTOR_MODEL="${VAL_EXECUTOR_MODEL:-}"
VAL_EXECUTOR_API_BASE="${VAL_EXECUTOR_API_BASE:-}"
VAL_EXECUTOR_API_KEY="${VAL_EXECUTOR_API_KEY:-}"
# build the optional val-executor hydra args ONLY if a val executor override was requested (avoids
# passing empty-string overrides that hydra could misparse).
VAL_EXEC_ARGS=()
...
# Executor thinking is separate (EXECUTOR_ENABLE_THINKING, already false).
