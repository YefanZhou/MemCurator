...
class MemoryGenerationManager:
    ...
    def _process_next_chunk(self, chunks: List[str], batch_memories: List[Memory]) -> torch.Tensor:
        ...
        for chunk, memory in zip(chunks, batch_memories):
            ...
            if token_ids.shape[1] > 6144:
                token_ids = token_ids[:, -6144:]
        ...
        if chunk_ids.shape[1] > 6144:
            chunk_ids = chunk_ids[:, -6144:]
    ...
    def run_memory_loop(self, gen_batch, questions_and_answers: List[List[dict]], data_sources: List[str], num_gpus: int = 1, global_steps: int = 0, is_validation: bool = False) -> DataProto:
        ...
        for question_idx in range(max_questions):
            ...
            step_questions, step_predictions = self._process_question_with_memory_using_server(
            ...
            for reordered_idx, i in enumerate(remaining_indices):
                ...
                current_chunks.append(self.prompts_wrt_datasource['unified_prompt'].format(context=chunk_content, max_new_tokens=int(self.config.max_response_length * 0.8)))
            ...
            if is_validation:
                dump_path = "/home/ouyangsiru/SkillCurator/data/math/generation/validation"
            else:
                dump_path = "/home/ouyangsiru/SkillCurator/data/math/generation/training"

            os.makedirs(dump_path, exist_ok=True)
            filename = os.path.join(dump_path, f"{global_steps}.jsonl")
            
            line_content = {
                "memories": [{'core': memory.core, 'episodic': memory.episodic, 'semantic': memory.semantic} for memory in active_memories],
