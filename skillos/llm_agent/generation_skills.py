...
# QUERY_PROMPT = """
# Now use your structured memory to assist in solving the following math problem. 
...
# ---
# **EXAMPLE FORMAT**:
# Memory Usage: [Your memory analysis here...]
# Reasoning: [Your step-by-step reasoning process here, grounded on memories provided...]
...
QUERY_PROMPT = """Now use your skills to assist in solving the following math problem. 

**Instruction**: 
- **In your response, you MUST first explicitly state which skills you used or why you didn't.**
- Follow the exact format provided in the example below.

---
**EXAMPLE FORMAT**:
Skills Usage: [Your skills usage analysis here...].
Reasoning: [Your step-by-step skills-grounded reasoning process here...]
Final Answer: [Your final answer here...]
---
...
# Instruction
First, analyze the execution and compare with past skills. Then generate tool calls to insert new skills, update existing skills, or delete outdated/irrelevant skills.
"""
...
class MemoryGenerationManager:
    ...
    def _process_next_chunk(self, chunks: List[str], batch_memories: List[SkillMemory]) -> torch.Tensor:
        """Process next chunks of information using memory agent template logic."""

        # Import the memory agent to use its processing logic
        from skills.skills_agent import SkillAgent
        from skills.skills_functions import MEMORY_TOOL_SCHEMAS, get_memory_tool_schemas
        ...
        for chunk, memory in zip(chunks, batch_memories):
            if chunk.strip():  # Only process non-empty chunks with full pipeline
                ...
                token_ids = SkillAgent.process_text_with_qwen_pipeline(
                    text=chunk,
                ...
                max_num_of_recent_chunks = SkillAgent.MAX_MEMORY_ITEMS
                while token_ids.shape[-1] > 16384 and max_num_of_recent_chunks > 0:
            ...
            # truncate token_ids if too long
            if token_ids.shape[1] > 16384:
                token_ids = token_ids[:, -16384:]

            processed_token_ids_list.append(token_ids)

        for token_ids in processed_token_ids_list:
            assert token_ids.shape[1] <= 16384, f"token_ids.shape[1]: {token_ids.shape[1]}"
        ...
        if chunk_ids.shape[1] > 16384:
            chunk_ids = chunk_ids[:, -16384:]

        assert chunk_ids.shape[1] <= 16384, f"chunk_ids.shape[1]: {chunk_ids.shape[1]}"
    ...
    def _process_question_with_memory_using_server(self, batch_memories, questions_and_answers):
        ...
        payload = {
            "memories": [{'skills': memory.skills} for memory in batch_memories],
        ...
        return questions_list, results, retrieved_skills
    ...
    def _analyze_function_call_content(self, function_calls: List[Dict]) -> List[float]:
        ...
        print(f"[DEBUG]: Sending to server: {json.dumps({'batch': function_calls[:1]}, indent=2)}")
    ...
    def run_memory_loop(self, gen_batch, questions_and_answers: List[List[dict]], data_sources: List[str], num_gpus: int = 1, global_steps: int = 0, is_validation: bool = False) -> DataProto:
        """Run memory agent loop to process questions and perform memory operations.
        
        The loop iterates through questions_and_answers. For each question:
        1. Use _process_question_with_memory_using_server to get predictions
        2. Form new chunks based on questions, predictions, and past skills
        3. Process chunks with _process_chunk_with_memory_operations to update memory
        """
        ...
        batch_memories = [SkillMemory() for i in range(batch_size * self.config.num_rollouts)]
        ...
        # Main memory processing loop - iterate through questions
        max_questions = max(len(qa_list) for qa_list in questions_and_answers) if len(questions_and_answers) > 0 else 0
        ...
        # Initialize QA reward storage
        all_step_questions_list = []
        all_step_predicted_answers_list = []
        all_memories_list = []

        for question_idx in range(max_questions):

            print(f"[DEBUG] Processing question {question_idx + 1}/{max_questions}")

            remaining_indices = []
            # Get current questions for active sequences
            step_qa_with_prompt = []
            for i, qa_list in enumerate(questions_and_answers):
                if question_idx < len(qa_list):
                    remaining_indices.append(i)
                    qa = qa_list[question_idx]
                    query_prompt = QUERY_PROMPT.format(question=qa['question'])
                    formatted_qa = [{
                        'question': query_prompt + "\n\n" + "Please reason step by step, and put your final answer within \\boxed{}.",
                        'answer': qa['answer'],
                    }]
                    step_qa_with_prompt.append(formatted_qa)
                else:
                    active_mask[i] = 0
            ...
            # Step 1: Get predictions using the server
            active_memories = [batch_memories[i] for i in remaining_indices]
            step_questions, step_predictions, step_retrieved_skills = self._process_question_with_memory_using_server(
                active_memories, step_qa_with_prompt
            )

            # Step 2: Form chunks from questions and predictions for memory operations
            current_chunks = []
            for reordered_idx, i in enumerate(remaining_indices):
                qa = questions_and_answers[i][question_idx]
                question = qa['question']
                # Use the first prediction from the server response
                prediction = step_predictions[reordered_idx][0] if step_predictions[reordered_idx] else ""
                past_skills = step_retrieved_skills[reordered_idx][0] if step_retrieved_skills[reordered_idx] else ""
                
                # Form the chunk as "###Question\n{question}\n\n###Attempted Solution\n{prediction}"
                # chunk_content = f"###Question\n{question}\n\n###Attempted Solution\n{prediction}"
                chunk_content = UNIFIED_PROMPT.format(question=question, past_skills=past_skills, model_execution=prediction)
                current_chunks.append(chunk_content)
                # current_chunks.append(UNIFIED_PROMPT.format(context=chunk_content, max_new_tokens=int(self.config.max_response_length * 0.8)))
            ...
            # Step 3: Process chunks with memory operations
            ...
            # Log generator outputs and memories for debugging
            if is_validation:
                dump_path = os.getenv("ROLLOUT_DATA_DIR") + "/generation/validation"
            else:
                dump_path = os.getenv("ROLLOUT_DATA_DIR") + "/generation/training"
            ...
            line_content = {
                ...
                'questions': [qa_list[question_idx]['question'] for qa_list in [questions_and_answers[i] for i in remaining_indices]],
                "predictions": [[sublist for sublist in item] for item in step_predictions],
                "gold_answers": [qa_list[question_idx]['answer'] for qa_list in [questions_and_answers[i] for i in remaining_indices]],
                "formed_chunks": current_chunks,
            }

            assert len(line_content["memories"]) == len(line_content["questions"]) == len(line_content["predictions"]) == len(line_content["gold_answers"])

            with open(filename, "a") as f:
                f.write(json.dumps(line_content, ensure_ascii=False) + "\n")

            # Gather step results into full batch format
            full_step_questions = [None] * batch_size
            full_step_predictions = [None] * batch_size
            full_memories = [None] * batch_size
            for reordered_idx, idx in enumerate(remaining_indices):
                full_step_questions[idx] = step_questions[reordered_idx]
                full_step_predictions[idx] = step_predictions[reordered_idx]
                full_memories[idx] = batch_memories[reordered_idx]

            all_step_questions_list.append(full_step_questions)
            all_step_predicted_answers_list.append(full_step_predictions)
            all_memories_list.append(full_memories)
        ...
        final_predicted_answers_list = []
        final_memories_list = []
        for idx in range(batch_size):
            final_questions_list.append([step[idx][0] for step in all_step_questions_list if step[idx]])
            final_predicted_answers_list.append([step[idx][0] for step in all_step_predicted_answers_list if step[idx]])
            final_memories_list.append([step[idx] for step in all_memories_list if step[idx]])

        questions_list = final_questions_list
        predicted_answers_list = final_predicted_answers_list
        memories_list = final_memories_list
    ...
    def _process_chunk_with_memory_operations(self, rollings: DataProto, current_chunks: List[str], batch_memories: List[SkillMemory]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        """
        ...
        memory_agent_template = SkillAgent(agent_config={'model_name': 'Qwen/Qwen3-4B', 'vllm': False}, is_template=True)
