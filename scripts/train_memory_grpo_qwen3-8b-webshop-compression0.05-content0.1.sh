...
exp_name="qwen3-8b-8b-skills-webshop-signal-freesec+content${function_content_reward_weight}+compression${compression_ratio_weight}"
...
# Data paths (webshop is env-based; tasks come from env.reset(), not data files)
export DATA_DIR='/home/siruo_google_com/SkillCurator/data/webshop'
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    ray job submit --runtime-env="${RUNTIME_ENV}" \
        ...
        +webshop.use_small=True \
        +webshop.human_goals=False \
        +webshop.num_tasks=10 \
        +webshop.val_tasks=20 \
        +webshop.max_steps=30 \
        +webshop.same_task_type_per_chain=false \
