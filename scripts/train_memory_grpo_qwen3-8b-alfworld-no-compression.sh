...
export LD_LIBRARY_PATH=/home/siruo_google_com/miniconda3/envs/memory/lib:$LD_LIBRARY_PATH

export QWEN_URL="http://10.148.0.45:8002/v1"

#!/usr/bin/env bash
set -xeuo pipefail

# Ablation: remove compression reward (compression_ratio_weight=0.0)
compression_ratio_weight=0.0
function_content_reward_weight=0.1
function_call_reward_weight=0.1
sub_sample_question_ratio=1.0

# Project and experiment configuration
project_name='SkillOS'
exp_name="qwen3-8b-8b-skills-alfworld-ablation-no-compression+content${function_content_reward_weight}+compression${compression_ratio_weight}"

# Memory-specific configurations
export WANDB_API_KEY="b7f2f63f1817334d8df0cf1902d43c8eca9f51ba"
export WANDB_PROJECT="${project_name}"
export WANDB_NAME="${exp_name}"
export BASE_MODEL='Qwen/Qwen3-8B'
export EXPERIMENT_NAME="${exp_name}"

# Data paths (alfworld is env-based; tasks come from env.reset(), not data files)
export DATA_DIR='/home/siruo_google_com/SkillCurator/data/alfworld'
export ROLLOUT_DATA_DIR="${DATA_DIR}/${exp_name}"
TRAIN_DATA_DIR='/home/siruo_google_com/SkillCurator/data/math/'
TEST_DATA_DIR='/home/siruo_google_com/SkillCurator/data/math/'
...
max_turns=5
respond_url="http://10.148.0.45:5000/batch_process"
analyze_function_url="http://10.148.0.45:5000/analyze_function"
...
WORKING_DIR="/home/siruo_google_com/SkillCurator"
RUNTIME_ENV=${RUNTIME_ENV:-"${WORKING_DIR}/verl/trainer/runtime_env.yaml"}
NNODES=${NNODES:-${PET_NNODES:-1}}

# Model and checkpoint paths
MODEL_PATH=${MODEL_PATH:-"${BASE_MODEL}"}
CKPTS_DIR=${CKPTS_DIR:-"/home/siruo_google_com/SkillCurator/ckpt/${exp_name}"}
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    ray job submit --runtime-env="${RUNTIME_ENV}" \
        ...
        trainer.rollout_data_dir="${ROLLOUT_DATA_DIR}/rollout/training" \
        trainer.val_only=false \
        trainer.val_before_train=false \
        trainer.validation_data_dir="${ROLLOUT_DATA_DIR}/rollout/validation"\
        ...
        +alfworld.num_tasks=10 \
        +alfworld.val_tasks=20 \
        +alfworld.max_steps=30 \
        +alfworld.same_task_type_per_chain=true \
